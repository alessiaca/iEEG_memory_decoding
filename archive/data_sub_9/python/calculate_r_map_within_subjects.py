# R_map calculation within subjects
...
def compute_corr_maps(subject, image_all, decoding_measure, df_decoding, model_name):

    channels = df_decoding[df_decoding["subject"] == subject]["channel"].unique()

    for channel in np.hstack((0, channels)):
        if channel == 0:
            idx_train = np.where(df_decoding["subject"] == subject)[0]
        else:
            idx_train = np.where((df_decoding["subject"] == subject) & (df_decoding["channel"] != channel))[0]
        ...
        savemat(f"{folder_path}/without_{channel}_func_seed_AvgR_Fz.mat", {"corr": corr, "p": p})
        ...
        nib.save(nii_new, f"{folder_path}/without_{channel}_func_seed_AvgR_Fz.nii")


# Use joblib.Parallel to parallelize the computation
Parallel(n_jobs=1)(delayed(compute_corr_maps)(i, image_all, decoding_measure, df_decoding, model_name) for i in subjects)
