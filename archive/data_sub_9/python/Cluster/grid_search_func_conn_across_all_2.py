...
def compute_similarity(df, decoding_mode, flip_mode, p_thresh, mask):
    ...
    df = df[(df["accuracy"] > 0.5) & (df["p_value"] < p_thresh)]
    ...
    # Loop over subjects and channels
    for idx_test, row in df.iterrows():
        subject = int(row["subject"])
        channel = int(row["channel"])

        # Leave one patient out
        idx_train = np.delete(np.arange(len(df)), idx_test)
        ...
        results_sub = []
        image_flat = image_all[idx_test].flatten()
        ...
        results_sub = [decoding_mode, subject_mode.mean(), flip_mode, overlap_thresh, mask, subject, channel, similarity, decoding_accuracy[idx_test]]
        ...
        # Write to database
        with sqlite3.connect(f"functional_connectivity_results_across_all/{decoding_mode}_{subject_mode.mean()}_{flip_mode}_{overlap_thresh}_{mask}.db", timeout=10) as conn:
