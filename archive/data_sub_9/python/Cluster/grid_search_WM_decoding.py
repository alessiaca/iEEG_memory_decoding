...
def run_cross_val(X, y, n_components):
...
def decode_WM_load(subject, channel):
    ...
    epochs = mne.read_epochs(f"../../../Processed_data/re_referenced/Subject_0{subject}_car_all.fif")
    ...
    for seconds, components, sampling_hz in combinations:
        ...
        n_runs = 200
        results = [[subject, channel+1, seconds, components, sampling_hz, run_cross_val(X, y, n_components=components), run, 0] for run in range(n_runs)]

        # Compute the performance of 200 runs with shuffled labels
        results_shuffles = [[subject, channel+1, seconds, components, sampling_hz, run_cross_val(X, shuffle(y, random_state=run), n_components=components), run, 1] for run in range(n_runs)]
