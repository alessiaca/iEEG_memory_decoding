...
def decode_WM_load(subject, channel):
    ...
    seconds = [1, 2, 3, 4, 5]
    sampling_hz = [1, 2, 3, 4, 5]
    combinations = list(product(seconds, sampling_hz))
    ...
    for seconds, sampling_hz in combinations:
        ...
        n_runs = 200
        results = [[subject, channel+1, seconds, sampling_hz, run_cross_val(X, y), run, 0] for run in range(n_runs)]

        # Compute the performance of 200 runs with shuffled labels
        results_shuffles = [[subject, channel+1, seconds, sampling_hz, run_cross_val(X, shuffle(y, random_state=run)), run, 1] for run in range(n_runs)]

        # Create a DataFrame from the collected results
        combined_results = results + results_shuffles
        results_df = pd.DataFrame(combined_results, columns=['subject', 'channel', 'seconds', 'sampling_hz', 'accuracy', 'run', 'shuffled'])
        
        # Write to database
        with sqlite3.connect(f"decoding_results_2/{subject}_{channel}.db", timeout=30) as conn:
