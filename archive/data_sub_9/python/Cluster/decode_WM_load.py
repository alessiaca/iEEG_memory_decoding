...
def run_cross_val(X):
    ...
    iterations=100,       # Fewer iterations for small data
    depth=3,              # Shallow trees to avoid overfitting
    learning_rate=0.05,
    ...
    for i in range(features_sub_chan.shape[1]):
        for k in range(n_runs):
            row = {
                ...
                'run': k,
                'shuffled': 0,
                'accuracy': run_cross_val(features_sub_chan[:, i, :3])
            }
            ...
            row = {
                ...
                'run': k,
                'shuffled': 1,
                'accuracy': run_cross_val(features_sub_chan[idx_perm, i, :])
            }
            decoding_accuracy_all.append(row)
